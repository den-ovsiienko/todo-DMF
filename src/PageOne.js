...
import { useAddTodoDummyMutation, useAddTodoMutation, useGetAllTodosQuery } from './services/todos';
...
const PageOne = () => {
  ...
  const [addTodoDummy] =  useAddTodoDummyMutation();
  ...
  // const addNew = () => {
  //   useAddTodoMutation({name: 'DUMMY'});
  // }


  const { data, error, isLoading } = useGetAllTodosQuery();

  console.log('ALL TODOs FROM PAGE', data)
  ...
  return (
    ...
    <div></div>
      ...
      <div><Link to='/anotherPage'>GO TO ANOTHER PAGE</Link></div>
      <button onClick={() => addTodoDummy({name: 'DUMMY'})}>Add DUMMY</button>
